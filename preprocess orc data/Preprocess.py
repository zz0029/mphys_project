...
# ========= Transform，必须和 RGZ/MB 一致 =========
mu, sig = 0.0039216, 0.019508    # 你可以从 byol.config["data"]["mu"] 取
...
for fits_file, cat_file in zip(fits_paths, catalogue_paths):
    ...
    # 每个文件只有 1 个 ORC
    img, label = ds[0]    # img shape: (1,70,70)

    # 确保 numpy → tensor
    if not torch.is_tensor(img):
        img = torch.from_numpy(img).float()

    # 应用 Normalize transform（和 RGZ/MB 保持一致）
    img = transform(img.squeeze())    # 去掉第1维 → 70x70
    img = img.unsqueeze(0)            # 再加回来 → (1,70,70)
